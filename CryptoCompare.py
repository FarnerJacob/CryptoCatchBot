...
URL_HISTO_MINUTE = 'https://min-api.cryptocompare.com/data/histominute?fsym={}&tsym=USD&limit=360&aggregate=3&e=CCCAGG&toTs={}'
URL_HISTO_HOUR = 'https://min-api.cryptocompare.com/data/histohour?fsym={}&tsym=USD&limit=24&aggregate=3&e=CCCAGG&toTs={}'
...
def get_histo_minute(coin, timestamp):
    if isinstance(timestamp, datetime.datetime):
        timestamp = time.mktime(timestamp.timetuple())
    response = query_cryptocompare(URL_HISTO_MINUTE.format(coin, timestamp),errorCheck=False)
    if response:
        return response

def get_histo_hour(coin, timestamp):
    if isinstance(timestamp, datetime.datetime):
        timestamp = time.mktime(timestamp.timetuple())
    response = query_cryptocompare(URL_HISTO_HOUR.format(coin, timestamp),errorCheck=False)
    if response:
        return response
